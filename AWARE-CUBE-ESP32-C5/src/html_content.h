/**
 * html_content.h — HTML/CSS/JS für das Captive Portal Web-Interface
 *
 * Komplett inline (kein externes CSS/JS), da im AP-Modus kein Internet verfügbar.
 * Mobilfreundliches, responsives Design.
 * Gesamtgröße: ca. 6KB (unter dem 20KB-Limit)
 */
...
function doConnect(){
  ...
  fetch('/connect',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},
    ...
    if(data.status==='connecting'){
      ...
      connectFail(data.reason||'Unbekannter Fehler');
    }
  }).catch(function(){
...
function pollStatus(){
  fetch('/status').then(r=>r.json()).then(data=>{
    if(data.status==='connecting')return; // weiter warten
    clearInterval(pollTimer);
    pollTimer=null;
    if(data.status==='ok'){
      connectOk(data.ip);
    } else {
      connectFail(data.reason||'Verbindung fehlgeschlagen');
    }
  }).catch(function(){
    // AP könnte schon abgeschaltet sein → Erfolg annehmen
    clearInterval(pollTimer);
    pollTimer=null;
    showStatus('ok','Gerät verbunden! Dieses Fenster kann geschlossen werden.');
    $('btnConnect').disabled=false;
  });
}

function connectOk(ip){
  showStatus('ok','Verbunden! IP: '+escHtml(ip)+'<br>Der Access Point wird gleich abgeschaltet.<br>Sie können sich jetzt mit Ihrem normalen WLAN verbinden.');
  $('btnConnect').disabled=false;
}

function connectFail(reason){
  var msg={'wrong_password':'Falsches Passwort','timeout':'Zeitüberschreitung — SSID erreichbar?',
    'no_ssid':'Netzwerk nicht gefunden','busy':'Bereits ein Verbindungsversuch aktiv'};
